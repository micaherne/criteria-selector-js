...
$.widget("strath.criterion", {
	...
	_receiveCriteriaData: function(data, self) {
		...
		if(recipients.length == 0) {
			...
			if(self._isEditable()) {
				var editButton = $(self.options.editor.icon('edit'))
					.click( function() {
						self._showEditor();
				});
				$(self.element).find('.criterion-controls').prepend(editButton).append($('<div/>').addClass('criterion-settings').html("Edit " + self.recipient.label));
			}
	...
	_showEditor: function() {
		var self = this;
		var positioningElement = $(self.element).find('.criterion-controls');
